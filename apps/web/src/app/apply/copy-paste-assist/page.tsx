 ...
 */
// next/navigation's useSearchParams() opts a page out of static rendering
// unless the component reading it sits under its own Suspense boundary
// (https://nextjs.org/docs/messages/missing-suspense-with-csr-bailout) —
// `next build`'s prerendering step fails without this split, caught only
// by a real production build (docker compose's `web` target), not by
// typecheck/eslint/unit tests.
export default function CopyPasteAssistPage() {
  return (
    <Suspense fallback={<Spinner />}>
      <CopyPasteAssistPageInner />
    </Suspense>
  );
}

function CopyPasteAssistPageInner() {
